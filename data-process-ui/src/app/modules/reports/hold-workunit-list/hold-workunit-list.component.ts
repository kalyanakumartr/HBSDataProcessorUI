...
@Component({
  selector: 'app-hold-workunit-list',
  templateUrl: './hold-workunit-list.component.html',
  styleUrls: ['./hold-workunit-list.component.scss']
})
export class HoldWorkunitListComponent implements   OnInit,
OnDestroy,
IDeleteAction,
IDeleteSelectedAction,
IFetchSelectedAction,
...
authModel:AuthModel;
  ...
  ngOnInit(): void {
    ...
    this.deliveryTrackerService.fetch("/workUnitHoldReport");
  ...
  filter() {
    ...
    this.deliveryTrackerService.patchState({ filter },"/workUnitHoldReport");
  }
  ...
  search(searchTerm: string) {
    this.deliveryTrackerService.patchState({ searchTerm },"/workUnitHoldReport");
  }
  SearchFilter(){
    ...
    this.deliveryTrackerService.patchState({ departmentId:this.department,divisionId:this.division,projectId:this.project,fromDate:fromDat,toDate:toDat},"/workUnitHoldReport");
  }
  setFromDate(){
  ...
  sort(column: string) {
    ...
    this.deliveryTrackerService.patchState({ sorting },"/workUnitHoldReport");
  }

  // pagination
  paginate(paginator: PaginatorState) {
    this.deliveryTrackerService.patchState({ paginator },"/workUnitHoldReport");
  }
  // form actions
  ...
  setDepartment(value){
    ...
    if(position.length>1){
      ...
      if(this.department != "0"){
       ...
       // this.deliveryTrackerService.patchState({ departmentId:this.department },"/workUnitHoldReport");
      }
    }
  }
  setDivision(value){
    ...
    if(position.length>1){
      ...
      if(this.division != "0"){
        this.getProjectForDivision();
       // this.deliveryTrackerService.patchState({ divisionId:this.division },"/workUnitHoldReport");
      }
    }
  }
  ...
  clearFilter(){
    ...
    if(this.isClearFilter){
      ...
      if(this.showDepartment){
        this.deliveryTrackerService.patchState({ },"/workUnitHoldReport");
      }else{
        this.deliveryTrackerService.patchState({ departmentId:this.department,divisionId:this.division,fromDate:this.fromDate,toDate:this.toDate},"/workUnitHoldReport");
      }
      this.grouping = this.deliveryTrackerService.grouping;
      this.paginator = this.deliveryTrackerService.paginator;
      this.sorting = this.deliveryTrackerService.sorting;
